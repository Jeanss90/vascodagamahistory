...
?>
    <div class="competition-box" id="group-fase group<?php echo $group?>">
        <!--Adding Tables-->
        <div>
            <table table class="competitions sudamericana">
                <thead>
                    <tr>
            ...
            <table class="competitions" colspan="11">
                ...
                <?php
                    ...
                    $sql = "SELECT jogos.id, horario, escudo_casa, casa, score_casa, penalti_casa, penalti_fora, score_fora, fora, escudo_fora, competicao, estadio, rodada, grupo
                        ...
                        WHERE competicao = 'Copa Sudamericana' AND grupo = '$group' AND horario > '$year-01-01' ORDER BY grupo, horario;";    // Select table here 
                    $result = mysqli_query($conn,$sql);  // here i am run the query
                    ...
                    #get last round
                    $sql = "SELECT * FROM jogos WHERE competicao = 'Copa Sudamericana' AND grupo = '$group' AND horario > '$year-01-01 00:00' ORDER BY id;";    // Select table here 
                    $result = mysqli_query($conn,$sql);  // here i am run the query
                    ...
                    $sql = "SELECT jogos.id, horario, escudo_casa, casa, score_casa, penalti_casa, penalti_fora, score_fora, fora, escudo_fora, competicao, estadio, rodada, grupo
                        ...
                        INNER JOIN brasao_casa ON brasao_casa.clube = jogos.casa)
                        INNER JOIN brasao_fora ON brasao_fora.clube = jogos.fora)
                        WHERE competicao = 'Copa Sudamericana' AND grupo = '$group' AND horario > '$year-01-01' ORDER BY grupo, rodada, horario;";    // Select table here 
                    $result = mysqli_query($conn,$sql);  // here i am run the query
                    ...
                    while($row = mysqli_fetch_assoc($result)) // Showing all the data
                    {
                        ...
                        if ($rodada_anterior <> $rodada) {
                            ...
                            if ($first_match) {
                                ...
                                echo"    
                                <tbody class='show' id='group".$group."_round".$rodada."'>";
                                } else {
                                echo"    
                                <tbody class='hidden' id='group".$group."_round".$rodada."'>"; 
                                }
                                echo"
                                <tr>
                                    <th style='display: flex; justify-content: space-between; align-items: center;'>";
                                        if ($rodada == 1) {
                                            ...
                                            echo"
                                            <span class='prev material-symbols-outlined' onmouseover='modeHover()' onclick='moveTablePrevGroup($group, $rodada)'>
                                                chevron_left       
                                            </span>";
                                        }
                                        echo"
                                        ...
                                        if ($rodada == $last_round) {
                                            ...
                                            echo"
                                            <span class='next material-symbols-outlined' onmouseover='modeHover()' onclick='moveTableNextGroup($group, $rodada)'>
                                                chevron_right        
                                            </span>";
                                        }
                                    echo"
